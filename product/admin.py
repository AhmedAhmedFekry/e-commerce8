...
class ProductAdmin(ImportExportModelAdmin):
    ...
    def show_start_page(self, request, queryset):
        ...
        if count > 1:
            ...
            self.message_user(
                request, f"{count} Product Is Showing In The Start Page ")
    ...
    def make_status_enable(self, request, queryset):
        ...
        if count > 1:
            self.message_user(request,
                              f"{count} Products are make status enable ")
        else:
            self.message_user(request,
                              f"{count} Product Is make status enable ")
    ...
    def make_status_disable(self, request, queryset):
        ...
        if count > 1:
            self.message_user(request,
                              f"{count} Products are make status disable ")
        else:
            self.message_user(request,
                              f"{count} Product Is make status disable ")
    ...
    fieldsets = (
        ...
        (
            ...
            {
                "classes": ("", ),
                "fields": ("category", "vendor", "price", "variant", "amount",
                           "minamount", "image", "image_tag", "status",
                           "slug_en", "slug", "slug_ar"),
            },
        ),
        (
            None,
